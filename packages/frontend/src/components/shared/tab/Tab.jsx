...
const Tab = ({ loading, size, tabs, selectedTab, onTabClick }) => {
  ...
  return (
    <div data-testid="tab-container" className={`flex flex-col justify-center mx-auto ${tabSize(size)}`}>
      <div className="w-full border-b border-background-500">
        {tabs.map((tab, i) => (
          <button
            key={i}
            className={`inline-flex items-center h-12 px-4 py-2 text-center whitespace-nowrap ${
              selectedTab === i
