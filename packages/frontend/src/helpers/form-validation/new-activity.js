...
export const stepOne = (values, verifyDates, setError, t) => {
  ...
  if (!validateRequired(values.planId, "new_activity_plans_required"))
    return false;
  if (!validateRequired(values.name, "new_activity_name_required")) return false;
  if (!validateLength(values.dates, "new_activity_dates_required")) return false;
  if (verifyDates()) return false;
  if (!validateCep(values.addressZipCode))
    return showError("account_address_zipcode_invalid");
  if (
    !validateRequired(values.addressState, "new_activity_address_state_required")
  )
    return false;
  if (!validateRequired(values.addressCity, "new_activity_address_city_required"))
    return false;
  if (
    !validateRequired(values.addressStreet, "new_activity_address_street_required")
  )
    return false;
  if (
    !validateRequired(
...
export const stepTwo = (values, setError, t) => {
  ...
  if (
    ...
    return false;
  if (!validateRequired(values.visitorGift, "new_activity_visitors_gift_required"))
    return false;
  ...
  if (
    !validateRequired(
      ...
      "new_activity_raffle_type_required"
    )
  )
    return false;
  if (
  ...
  if (values.raffle && values.raffle === "YES") {
    if (!validateRequired(values.raffleType, "new_activity_raffle_type_required"))
      return false;
  }
  return true;
};
