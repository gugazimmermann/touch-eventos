...
export const handler: APIGatewayProxyHandlerV2WithJWTAuthorizer = async (
  ...
  try {
    ...
    let query = db
      ...
      .select(["registrationId", "confirmed", "activities_register.language"]);
    if (data?.phone) {
    ...
    const verifyRegister = await query.execute();
    ...
    // not confirmed
    if (!verifyRegister[0].confirmed) {
      return error(400, "Not Confirmed");
    }

    // generate confirmation code
    const code = (verifyRegister[0].registrationId.match(/\d/g) || [])
      .reverse()
      .join("")
      .slice(0, 6);
    ...
    await db
      ...
      .where("registrationId", "=", verifyRegister[0].registrationId)
      .executeTakeFirst();

    return {
      statusCode: 200,
      body: JSON.stringify({
        registrationId: verifyRegister[0].registrationId,
        language: data.language,
      }),
    };
  } catch (err) {
