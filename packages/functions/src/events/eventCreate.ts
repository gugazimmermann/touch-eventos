...
export const handler: APIGatewayProxyHandlerV2WithJWTAuthorizer = async (
  ...
  if (!eventsTable || !paymentsTable || !plansTable || !usersSubscriptionTable)
  ...
  try {
    const eventParams: PutCommandInput = {
      TableName: eventsTable,
      Item: data,
    };
    await dynamoDBClient.send(new PutCommand(eventParams));
    ...
    if (data.payment) {
      ...
      if (plan.type === "Múltiplos Eventos") {
      ...
      if (currentSubscription) {
        ...
        const updateEventParams: UpdateCommandInput = {
          TableName: eventsTable,
        ...
        await dynamoDBClient.send(new UpdateCommand(updateEventParams));
      }
    }
